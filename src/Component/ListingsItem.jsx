...
const ListingItem = ({listing , id})=>{
    ...
    return<li className="relative m-[10px] bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 ">
        <Link className="contant" to={`/category/${listing.type}/${id}`}>
            ...
            <div className="w-full p-[10px]">
                ...
                <div className="flex justify-between text-[#457b9d] mt-2 font-semibold">
                    { 
                        listing.offers ? (<p>${listing.discprice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")  } <del> ${listing.offers &&listing.regprice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</del> </p>)
                                        :  (<p>${listing.regprice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")  }</p>)
                    }
                    {listing.type === 'rent' && (<p className="text-red-500">    /month</p>)}
                </div>
